...
public class WeatherAPITest extends TestBase {
	...
	/**
	 * A test method which validates timezone from response
	 */
	@Test(priority=3,dependsOnMethods ="validateResponseCodeTest" )
	public void validateTimezoneTest()
	{
		
		String actualTimezone=WeatherAPIClient.getTimeZone(jsonResponse); 
		Assert.assertEquals(actualTimezone,properties.getProperty("timeZone"),"API response showing incorrect timezone"); //Assert timezone
		log.info("Timezone =   " +actualTimezone);
	}
	 ...
	 * requirement.
	 */
	@Test(priority = 4, dependsOnMethods = "validateResponseCodeTest")
